...
get_right(array(1, 2));
	...
	if(isset($_POST['addstd']) && $_POST['addstd']=='Save')
	{
		...
		else if(checkavailability('products', 'BarCode', $BarCode) > 0 || $BarCode == '') $msg = '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Please Enter a Unique Bar Code</div>';
...
curl_close($ch) ; 
			...
			if(isset($_FILES["File"]) && $_FILES["File"]['name'] != "")
			{
				...
				{			
					...
					$query2="UPDATE products SET Image='" . dbinput($realName2) . "' WHERE  ID=" . (int)$ProductID;
				...
				else
				{
					$msg='<div class="alert alert-warning alert-dismissable">
						<i class="fa fa-ban"></i>
						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						<b>Product has been added but Image can not be uploaded.</b>
						</div>';
				}
			}
			else
...
<?php if(isset($_SESSION["msg"]) && $_SESSION["msg"] != "")  { echo $_SESSION["msg"]; $_SESSION["msg"]=""; } ?>
              <div class="box box-default">
                ...
                <div class="box-body">
                    <div class="form-group">
						...
						<div class="col-md-6">
							<input readonly="" type="text" class="form-control" id="example-text-input" value="<?php $i = mysql_fetch_array(mysql_query("SELECT MAX(ID) AS MA FROM products")); echo $i["MA"]+1;?>" placeholder="" name="ProductID">
					  ...
					  <div class="form-group">
						...
						<div class="col-md-6">
							<input type="date" class="form-control" id="example-text-input" value="<?php echo $ManufacturingDate;?>" name="ManufacturingDate">
						</div>
					</div>
                    <div class="form-group">
						<label class="col-md-3 control-label" for="example-text-input">Serial Number</label>
						<div class="col-md-6">
							<input type="text" class="form-control" id="example-text-input" value="<?php $i = mysql_fetch_array(mysql_query("SELECT MAX(ID) AS MA FROM products")); echo $i["MA"]+1;?>" placeholder="" name="BarCode">
              ...
              <!---      <div class="form-group">
					<label class="col-md-3 control-label" for="example-text-input">T.W Category</label>
						<div class="col-md-6">
							<select class="form-control select2" multiple="multiple" data-placeholder="Select The Category(s)" name="Categories[]" style="width: 100%;">
								<?php
									...
									if($n == 0)
									{
										echo '<option value="0">No Categories Added</option>';
									}
									else
									{
										while($Rs = mysql_fetch_assoc($r)) { ?>
										<option value="<?php echo $Rs['ID']; ?>" <?php foreach($CategoryID as $c) { if($c==$Rs['ID']) { echo 'Selected=""'; } } ?>><?php echo $Rs['Name']; ?></option>
										<?php }
									}
							?>
							</select>
                    ...
                    <div class="form-group">
						...
						<div class="col-md-6">
							<input type="text" class="form-control" id="example-text-input" value="<?php echo $ShortDescription;?>" placeholder="Enter Short Description" name="ShortDescription">
						</div>
					</div>
