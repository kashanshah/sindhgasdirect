...
$query="SELECT p.ID, p.RefNum, pr.ProductName, u.UserName, p.Quantity, p.Price, p.TotalPrice, p.IniPayment, p.RemPayment, DATE_FORMAT(p.DateAdded, '%D %b %Y %r') AS DateAdded FROM purchases p JOIN products pr ON p.ProductID=pr.ID JOIN users u ON p.SupplierID = u.ID WHERE p.ID=".$ID;
// echo $query;
// exit();
...
$pdf->Output(DIR_PURCHASE_INVOICE2 . 'pinv' . $row["ID"] . '-' . $RefNumber . '.pdf', 'F');
			?> <script>alert('Purchase Added'); window.location.href = "../../Purchases.php";</script> <?php 
