...
//	if(!isset($_POST["CusomerID"])) $NewOldCustomer = 0;
    ...
    if ($msg == "") {
        $query1 = "INSERT INTO invoices SET DateAdded = '" . DATE_TIME_NOW . "', DateModified='" . DATE_TIME_NOW . "',
			PerformedBy = '" . (int)$_SESSION["ID"] . "',
			IssuedTo = '" . (int)$HandedTo . "',
			Note = '" . dbinput($Note) . "'";
        mysql_query($query1) or die(mysql_error('asd'));
        $InvoiceID = mysql_insert_id();

        $Unpaid = ($TotalAmount - ($Balance * GAS_RATE) - $Paid) < 0 ? 0 : ($TotalAmount - ($Balance * GAS_RATE) - $Paid);
        $Balance = getValue('users', 'Balance', 'ID', $_SESSION["ID"]);
        ...
        mysql_query("UPDATE purchases SET RefNum='" . generate_refno($InvoiceID) . "' WHERE ID=" . $InvoiceID) or die(mysql_error('a'));
        $i = 0;
        $TotalWeight = 0;
        $TotalPrice = 0;
        foreach ($CylinderID as $CID) {
            ...
            $CylinderType = getValue('cylinders', 'CylinderType', 'ID', $CID);
            $Capacity = getValue('cylindertypes', 'Capacity', 'ID', $CylinderType);
            $Rate = getValue('cylindertypes', 'Rate', 'ID', $CylinderType);
            $Price = $Rate * $Capacity;
            ...
            $query4 = "INSERT INTO purchase_details SET DateAdded = '" . DATE_TIME_NOW . "', DateModified='" . DATE_TIME_NOW . "',
				...
				CylinderID='" . (int)$CID . "',
				TierWeight='" . financials($CylinderWeight[$i]) . "',
				CompanyTotalWeight='" . financials($CompanyTotalWeight[$i]) . "',
				TotalWeight='" . financials($CurrentCylinderWeight[$i]) . "',
				Price='" . financials($Price) . "',
				ReturnStatus=0,
				ReturnWeight=0,
				ReturnDate='" . DATE_TIME_NOW . "',
				RetailPrice='" . financials($Price) . "',
				GasRate='" . financials(($Price / $Capacity)) . "',
				PerformedBy = '" . (int)$_SESSION["ID"] . "'
				";
            mysql_query($query4) or die(mysql_error('a'));
            $query2 = "INSERT INTO cylinderstatus SET DateAdded = '" . DATE_TIME_NOW . "',
				InvoiceID='" . (int)$InvoiceID . "',
				CylinderID='" . (int)$CID . "',
				HandedTo='" . (int)$HandedTo . "',
				Weight='" . financials($CurrentCylinderWeight[$i]) . "',
				PerformedBy = '" . (int)$DriverID[$i] . "'
			";
            mysql_query($query2) or die(mysql_error('a'));
            $i++;
        }

        $Balance = ($TotalWeight <= $Balance ? $TotalWeight : $Balance);
        $GasRate = $TotalPrice / $TotalWeight;
        $Unpaid = ($TotalPrice - ($Balance * $GasRate) - $Paid) <= 0 ? 0 : ($TotalPrice - ($Balance * $GasRate) - $Paid);

        $query4 = "INSERT INTO purchases_amount SET DateAdded='" . DATE_TIME_NOW . "', DateModified='" . DATE_TIME_NOW . "',
				PerformedBy = '" . (int)$_SESSION["ID"] . "',
        ...
        $PurchaseAmountID = mysql_insert_id();
        ...
        mysql_query("UPDATE purchases SET 
            ...
            GasRate = '".financials($GasRate)."'
            WHERE ID = '".(int)$InvoiceID."'
        ") or die(mysql_error('a'));
        mysql_query("UPDATE users SET Balance = Balance-" . financials($Balance) . " WHERE ID=" . $_SESSION["ID"]) or die(mysql_error('a'));

        $CylinderCount = $i;
        sendUserSMS($_SESSION["ID"], 'Recieved - ' . getValue('users', 'Name', 'ID', $DriverID[$i - 1]) . ' has delivered ' . $CylinderCount . ' cylinder(s) at ' . $_SESSION["Name"] . ' with total gas of ' . $TotalWeight . 'KG at ' . date('h:iA d-m-Y'));
        sendUserSMS($DriverID[$i - 1], 'Delivered - ' . $CylinderCount . ' cylinder(s) has been delivered to ' . $_SESSION["Name"] . ' with total return gas of ' . $TotalWeight . 'KG at ' . date('h:iA d-m-Y'));
        ...
        redirect("purchaseaddinvoice.php?ID=" . $PurchaseAmountID);
    }
    $_SESSION["msg"] = $msg;
}
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-12">
                                        <select name="CylinderID" id="CylinderID" class="form-control">
                                            <?php
                                            $r = mysql_query("SELECT ID, BarCode, TierWeight FROM cylinders WHERE Enabled=1 AND ExpiryDate > '" . date('Y-m-d h:i:s') . "' AND PlantID=" . (int)$_SESSION["PlantID"]) or die(mysql_error());
                                            $n = mysql_num_rows($r);
                                            if ($n == 0) {
                                                ...
                                                while ($Rs = mysql_fetch_assoc($r)) {
                                                    if (getCurrentStatus($Rs["ID"]) == ROLE_ID_DRIVER) {
                                                        ...
                                                        <option
                                                                ...
                                                                data-tierweight="<?php echo financials($Rs["TierWeight"]); ?>"
                                                                data-weight="<?php echo financials(getCurrentWeight($Rs["ID"])); ?>"
                                                                BarCode="<?php echo $Rs["BarCode"]; ?>"
                                                                value="<?php echo $Rs['ID']; ?>"
                                                            <?php if ($CylinderID == $Rs['ID']) {
                                                                echo 'selected=""';
                                                            } ?>>
                                                            <?php echo $Rs['BarCode']; ?>
                                                            - <?php echo $Rs['TierWeight'] ?>kg
                                                        </option>
                                                        <?php
                                                    }
                                                }
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="box box-body">
                                            <div class="form-group">
                                                <h3 class="col-md-12 control-label"
                                                    style="text-align: center;margin-top:0;" for="example-text-input">
                                                    Confirm adding <span id="cylinderCount">0</span> cylinders to stock?
                                                </h3>
                                            </div>
                                            <div class="form-group hidden">
                                                <label class="col-md-3 control-label" for="example-text-input">Payable
                                                    Amount</label>
                                                <div class="col-md-8">
                                            ...
                                            <div class="form-group hidden">
                                                ...
                                                <div class="col-md-8">
                                                    <input type="number" step="any" class="form-control" placeholder=""
                                                           ...
                                                           value="<?php echo (float)$Balance; ?>" id="Balance"
                                                           max="<?php echo (float)$Balance; ?>">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <input type="submit" name="addsale" value="Save changes"
...
<script>
    ...
    $("#AddItemButton").click(function () {
        ...
        if (j != 1) {
            ...
            $(".cart_table .DivCartCylinder" + $("[name='CylinderID']").val() + "").append('	<td>' + financial(gasWeight) + 'KG</td>');
