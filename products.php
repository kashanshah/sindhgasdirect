...
get_right(array(1, 2));
	...
	if(isset($_REQUEST['ids']) && is_array($_REQUEST['ids']))
	{
		...
		{
			//echo $CID;exit();
			$query = "DELETE FROM products WHERE ID = ".$CID."";	
			mysql_query($query);
			$_SESSION["msg"] = '<div class="alert alert-danger alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <h4><i class="icon fa fa-ban"></i> Product(s) Deleted!</h4>
                  </div>';
		}
	}
	if(isset($_REQUEST['DID']))
	{
			$query = "DELETE FROM products WHERE ID = ".$_REQUEST['DID']."";
			mysql_query($query);
			$_SESSION["msg"] = '<div class="alert alert-danger alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <h4><i class="icon fa fa-ban"></i> Product Deleted!</h4>
                  </div>';
			redirect($self);
	}
	if(isset($_REQUEST['TID']))
	{
			$query = "UPDATE products SET Status = IF(Status=1, 0, 1) WHERE ID='".(int)$_REQUEST['TID']."'";
			mysql_query($query);
			$query = "UPDATE products SET Featured = IF(Status=0, 0, IF(Featured=1, Featured, Featured)) WHERE ID='".(int)$_REQUEST['TID']."'";
			mysql_query($query);
			$_SESSION["msg"] = '<div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <b><i class="icon fa fa-ban"></i> Product Status Toggeled!</b>
                  </div>';
			redirect($self);
	}
	if(isset($_REQUEST['FID']))
	{
			$query = "UPDATE products SET Featured = IF(Featured=1, 0, 1) WHERE ID='".(int)$_REQUEST['FID']."'";
			mysql_query($query);
			$_SESSION["msg"] = '<div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <b><i class="icon fa fa-ban"></i> Product Featured Status Toggeled!</b>
                  </div>';
			redirect($self);
	}
...
<html>
  <head>
    ...
    <title><?php echo SITE_TITLE; ?>- Products</title>
              ...
              <div class="box">
                <div class="box-header">
                      <div class="btn-group-right">
                       ...
                       <button style="float:right;;margin-right:15px;" type="button" class="btn btn-group-vertical btn-success" onClick="location.href='addproduct.php'" data-original-title="" title="">Add Product</button>
                        ...
                        <td>
							<div class="btn-group">
							  ...
							  <ul class="dropdown-menu" role="menu">
								<li><a href="editproduct.php?ID=<?php echo $row["ID"]; ?>">Edit</a></li>
								<li class="divider"></li>
								<li class="divider"></li>
